...
def old_get_test_batches(stdfeat):
...
#     print(theano.sandbox.cuda.cuda_enabled)
#     # theano.sandbox.cuda.use('gpu3')
...
#                 print('[Queue {}] Start {} {} batches.\t{}'.format(idx, dt_type, len(bts), utils.print_time()))
#                 lg = get_batch_length(batch_queue[0])
#                 st = time.time()
#                 n_elem = 0
#                 n = shared_param.finished_num
#                 if shared_param.model:
#                     lasagne.layers.set_all_param_values(network, shared_param.model)
#                 for bt in batch_queue:
#                     if type(bt) != type('end'):
#                         if dt_type == 'train':
#                             proc_train_a_batch(bt, train_fn, shared_param)
#                         elif dt_type == 'val':
#                             proc_val_a_batch(bt, val_fn, shared_param)
#                         n_elem += 1
#                     else:
#                         n += 1
...
#     print('[Queue {}] Loaded\t{}\tbatches from {}.\t{:.4f} secs. ()'.format(i, len(bts), fn, time.time()-st))
#     del bts
...
def generate_fp_list(data_type, stdfeat, shuffle=False):
    ...
    if shuffle: 
        random.shuffle(indices)
    for i in indices:
        # if str(i) not in cache_data_dict[data_type]:
        fn = data_type + '_' + str(i) + '.npy'
        fp_list.append(os.path.join('var/stdfeat', stdfeat, fn))
    return fp_list
...
def proc_run(lock, idx, fp_list, shared_param, dt_type):
    import aed_class_run as run
    run.proc_run(lock, idx, fp_list, shared_param, dt_type)

def gen_procs(n_cores, model_lock, fp_list, shared_param, dt_type):
    proc_list = []
    for i in range(n_cores):
        fpl = fp_list[i::n_cores]
        name = 'loadfile_proc_'+str(i)
        proc = Process(target=proc_run, args=(model_lock, i, fpl, shared_param, dt_type),
                        name=name)
        # proc = Thread(target=proc_run, args=(queue, lock, idx, fpl),
        #                 name=name)
        proc.start()
...
def proc_test(shared_param, stdfeat, testing_type, frame_level='', print_prob=False):
    import aed_class_run as run
    run.proc_test(shared_param, stdfeat, testing_type, frame_level, print_prob)

def gen_test_proc(shared_param, stdfeat, testing_type, frame_level='', print_prob=False):
    proc = Process(target=proc_test, args=(shared_param, stdfeat, testing_type, frame_level, print_prob),
                     name='test_process')
    proc.start()
    print('Start testing...')
    proc.join()
